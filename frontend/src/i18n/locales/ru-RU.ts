export default {
  ...
  "collapse": "Свернуть",
  "expand": "Развернуть",
  "leaveHint": "Загрузка продолжается при переходах в приложении. Обновление или закрытие страницы прерывает очередь. Повторный выбор сначала проверяет дубликаты.",
  "total": "Файлов: {count}",
  "queued": "Ожидание предыдущей партии",
  "checkingHint": "Проверка отпечатков {count}/{total}; затем загрузка",
  "progress": "Прогресс партии",
  "receivedCount": "Загружено: {count}",
  "duplicateCount": "Дубликатов отклонено: {count}",
  "failedCount": "Ошибки / не подтверждено: {count}",
  "cancelledCount": "Остановлено: {count}",
  "removedCount": "Удалено: {count}",
  "stop": "Остановить",
  "retry": "Проверить и повторить",
  "rollback": "Отменить эту партию",
  "rollbackConfirm": "Удалить {count} документов, созданных этой партией? Существующие дубликаты и запросы с неподтверждённым результатом исключены.",
  "dismiss": "Скрыть запись",
  "files": "Состояние файлов",
  "parsing": "Обработка: ",
  "statusUnknown": "Статус не подтверждён",
  "batchDuplicate": "Повторное содержимое в партии; отклонено.",
  "serverDuplicate": "В базе уже есть файл этого типа с таким содержимым; отклонено.",
  "uncertainHint": "Запрос прерван, но сервер мог получить файл. Повторная попытка сначала проверяет дубликаты.",
  "preflightFailed": "Предварительная проверка не удалась; загрузка не продолжена.",
  "statusUnavailable": "Не удалось обновить статус обработки. Повторим позже.",
  "rollbackFailed": "Не все удаления выполнены. Проверьте и отмените ещё раз.",
  "rollbackPending": "Удаление подтверждено не полностью. Повторная отмена проверит оставшиеся документы.",
  "state": {
    "waiting": "Ожидание",
    "checking": "Проверка дубликатов",
    "uploading": "Загрузка",
    "uploaded": "Загружено",
    "duplicate": "Дубликат отклонён",
    "failed": "Ошибка",
  ...
  createChat: {
    title: 'Привет, я AutoLinta — автомобильные знания для точных решений',
  ...
  auth: {
    ...
    registerSubtitle: 'Создайте аккаунт и начните работу с AutoLinta Intelligence',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и AutoLinta автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.'
      },
      welcome: {
        title: 'Добро пожаловать в AutoLinta',
